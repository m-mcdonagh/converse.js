...
Users will however still have the ability to render individual audio via the message actions dropdown.
If you want to disallow users from doing so, set this option to an empty array ``[]``.
...
Users will however still have the ability to render individual videos via the message actions dropdown.
If you want to disallow users from doing so, set this option to an empty array ``[]``.
...
.. note::
  ...
  the ``show_images_inline``, ``embed_audio`` and ``embed_video`` settings.
